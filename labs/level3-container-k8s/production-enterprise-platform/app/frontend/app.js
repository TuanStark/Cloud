...
// Tự động nhận diện host:
// Nếu chạy sau Ingress (cổng 80, 443 hoặc rỗng), dùng đường dẫn tương đối để route qua Ingress.
// Nếu chạy dev local, fallback về cổng 5000.
const API_BASE_URL = (window.location.port === '' || window.location.port === '80' || window.location.port === '443')
  ? ''
  : (window.location.port === '5000' ? window.location.origin : `http://${window.location.hostname}:5000`);
